...
export default function InboundPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const data = Array.isArray(result.data)
        ? result.data
        : result.data.items ?? DEMO_INBOUND;
      setRows(data);
      setDemo(result.demo);
    })();
    return () => {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Inbound"
        description="Receiving queue and load history for the selected warehouse network."
        actions={
          canCreate ? (
            ...
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-[var(--surface)]/80">
                <td className="px-4 py-3">
                  <span className="font-[family-name:var(--font-mono)] font-medium text-[var(--brand-ink)]">
                    {row.loadNumber}
                  </span>
                  {row.carrier ? (
